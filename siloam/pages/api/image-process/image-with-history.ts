...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const { imageURL, convoHistory, request } = req.body as { imageURL: string, convoHistory: Message[], request: string};
    ...
    try {
        ...
        const userRequest: Message[] = [
                {
                    ...
                    content: [
                        { 
                            type: "text", 
                            text: request ? `<DESCRIBE_WITH_QUESTION> Answer this question directly with respect to the image provided. Answer to the best of your abilities if the request is not related to the image: "${request}"` : "<DESCRIBE>"
                        },
                        {
                            type: "image_url",
                            image_url: {
        ...
        const wholeConvo = systemPrompt.concat(convoHistory, userRequest);
